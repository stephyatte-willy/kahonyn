...
export default function Profile() {
  ...
  if (loading || authLoading) {
    return <ProfileLayout title="Mon Profil" activeTab="profile"><div className="flex items-center justify-center h-64"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[#FF6B35]"></div></div></ProfileLayout>
  }
  ...
  const memberSince = profile.createdAt ? new Date(profile.createdAt).toLocaleDateString('fr-FR', { year: 'numeric', month: 'long', day: 'numeric' }) : 'Date inconnue'
      ...
      <div className="space-y-6 max-w-4xl mx-auto">
          ...
          <div className="flex items-center gap-4">
            <button onClick={() => setIsAvatarModalOpen(true)} className="relative group flex-shrink-0">
              <div className="w-20 h-20 rounded-full bg-gradient-to-br from-[#FF6B35] to-[#FF8C5A] flex items-center justify-center text-2xl font-bold text-white overflow-hidden border-2 border-[#FF6B35]/30">
                {profile.avatar ? <img src={profile.avatar} alt="Avatar" className="w-full h-full object-cover" /> : <span>{displayInitial}</span>}
              </div>
        ...
        <div className="bg-[#1A1A2E] rounded-2xl p-6 border border-white/[0.04]">
          ...
          <div className="space-y-3">
            ...
            {isCreator && <ProfileField icon={PencilIcon} label="Bio" value={formData.bio} editing={editing} onChange={(val) => setFormData({ ...formData, bio: val })} multiline />}
          </div>
